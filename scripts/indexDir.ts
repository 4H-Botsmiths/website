...
//@ts-ignore
const tree = dirTree(path.join(__dirname, "../src/assets/images"), { exclude: /map.json|.DS_Store|map.data.json/ });
...
generateLink(tree).then(children => fs.writeFileSync(path.join(__dirname, "../src/assets/images/map.data.json"), JSON.stringify(children, null, 2).split(path.join(__dirname, '../src').split('\\').join('\\\\')).join('').split('\\\\').join('/')));
